...
int main(int argc, char* argv[])
{
    ...
    insert_btree(); // 주어진 두 개의 회사명 파일 안의 모든 회사명을 B+tree에 저장한다
    ...
    while (1) //test 단계
    {
        ...
        if (strcmp(command, "ex") == 0) //프로그램을 종료한다.
        {
            break;
        }
        else if (strcmp(command, "sp") == 0) //B+ tree안의 모든 레코드들을 키의 오름차순으로 출력
        {
...
type_ptr_datanode get_datanode(char* name)
{
    ...
    parent = curr; //리프노드 저장 (스택에 넣지는 않는다.)
    curr_d = curr->ptrd[i];
...
int range_search(char* key1, char* key2)
{
    ...
    if (ptr1 == ptr2)
    { // start key 와 end key 가 동일 데이타노드에 속한다.
...
{   //하나의 레코드를 삽입.
    ...
    do
    {
        ...
        if (curr->fill_cnt < MAXD_idx)   // curr 에 빈자리가 있다. 이 curr 노드에 넣는다.
        {
            ...
            { // i 부터 우측 부분을 포인터와 키를  한칸씩 우로 시프트 한다.
            ...
            if (curr->ptri[0] == NULL)
            {   // curr 는 리프노드이므로 type-1 삽입이다.
                curr->ptrd[i + 1] = child_d;   // 자식 포인터를 ptrd 에 삽입
                curr->ptri[i + 1] = NULL;
            }
            else
            {  // curr가 리프노드가 아님. 즉 split & elevation을 수행하고 온 경우임
                curr->ptri[i + 1] = child;
                curr->ptrd[i + 1] = NULL;
            }
        ...
        else    //   curr 에 빈자리가 없다. 인덱스노드의 빅노드 이용하여 split & elevation 을 수행한다.
        {
            ...
            {    // curr 가 ROOT 이다. 부모가 없다. 부모를 만들고 그것이 부모가 되어야 한다.
...
void sequential_print()
{
    ...
    while (datanode)
    {
        for (i = 0; i < datanode->fill_cnt; i++)
        {
            fprintf(fp, "%s %d\n", datanode->rec[i].name, datanode->rec[i].leng);
        }
        datanode = datanode->link;
    }
