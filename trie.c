...
nodeptr hang_down(char key[], nodeptr tn, int i)
{
    ...
    newn = (nodeptr)malloc(sizeof(nodetype));
    newn->ch = '\0'; //end of marker sign
    newn->bp = 0;
    newn->right = NULL;
    newn->below = NULL;
    tn->below = newn;
    // Trie에서 key의 마지막 노드 ('\0’를 가진 노드)는 데이타 화일의 해당 레코드 위치 (byte address) 를 가지고 있도록 한다.
    return newn;
} // hang_down
...
nodeptr insert_trie(char key[])
{
    int res, i;
    nodeptr tn;
    nodeptr curr = NULL, prev = NULL, tmp = NULL; // used to indicate insertion position

    // key를 TRIE에서 탐색하여 본다. 탐색이 성공하면 0 이상 값을 반환, 실패하면 -1을 반환하며 다음정보도 반환 받는다.
    res = search_trie(key, &curr, &prev, &i);

    if (res > 0) // already exists, thus exit with error
    {
        exit(1);
    }

    if (ROOT_TRIE == NULL) //nothing in the trie. insert the first one
    {
        ...
        tmp = hang_down(key, tn, 1); //나머지 단어 연결

    }
    else if (prev == NULL) //case 0 첫글자가 key보다 클때
    {
        tn = (nodeptr)malloc(sizeof(nodetype));
        tn->ch = key[i];
        tn->right = ROOT_TRIE;
        ...
        tmp = hang_down(key, tn, i + 1);

    }
    else if (prev->below == curr && curr != NULL) // case 1
    {
        tn = (nodeptr)malloc(sizeof(nodetype));
        tn->ch = key[i];
        prev->below = tn;
    ...
    else if (prev->right == curr && curr != NULL) //case 2
    {
        tn = (nodeptr)malloc(sizeof(nodetype));
        tn->ch = key[i];
        prev->right = tn;
    ...
    else if (prev->right == curr && curr == NULL) //case 3
    {
        tn = (nodeptr)malloc(sizeof(nodetype));
        tn->ch = key[i];
        prev->right = tn;
...
void substitution(nodeptr root_sub_trie, char key[], char  corr_key[], int ki, int ci, int n_s, int n_d, int n_i, int n_t)
{
    ...
    {
        start_ptr = np->below;
        new_corr_key[ci] = np->ch; // the character used for correction is for key[ki].
        new_corr_key[ci + 1] = '\0';
...
void deletion(nodeptr root_sub_trie, char key[], char  corr_key[], int ki, int ci, int n_s, int n_d, int n_i, int n_t)
{
    ...
    do
    {
        ...
        new_corr_key[ci + 1] = '\0';
        //모든 trie 조각에 대하여 남은 key의 suffix 스트링에 대한 매칭을 시도한다.
        if (start_ptr)
        {
...
void find_with_spell_correction(nodeptr root_sub_trie, char key[], char corr_key[], int ki, int ci, int n_s, int n_d, int n_i, int n_t)
{
    ...
    if (found_perfect_match == 0 && n_s + n_d + n_i + n_t <= 2)
    {
...
void sort_results(ty_correction cwords[])
{
    ...
    for (i = 0; i < nres - 1; i++)
    {
        ...
        for (j = i + 1; j < nres; j++)
        {
            ...
            {
                minval = cwords[j].penalty; midx = j;
            }
        }
