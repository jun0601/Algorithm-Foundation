/*
HW3_Hashing
컴퓨터정보통신학과 2019253018 이준형 visual stdio이용
*/
...
float compute_average_number_of_probes_per_search() 
{
    ...
    fp = fopen("Companies_Korean.txt", "r");   // 파일 여는 부분
    while (1)
    {
...
int del_middle(int s, int p, int* chain_split)
{
    ...
    if (found == -1)
    { // 찾기 실패. 위치 s 를 체인에서 skip 하고, s 의 레코드를 제거한다.
    ...
    { // s 를 home address 로 하는 원소가 s 이후의 체인에 존재한다.
       ...
       //       s 의 후행자부터 시작하여 HA 가 D(s) 에 속하지 않는 마지막 레코드를 찾자.
        ...
        { // s 다음 부터의 모든 원소는 home address 가 D(s)에 속함.
...
int main(void)
{
    ...
    fp = fopen("Companies_Korean.txt", "r"); //compnies korean
    while (1)
    {
        res = fgets(line, 300, fp);
        if (!res)
    ...
    while (1) 
    {
        ...
        while (line[k] != '\0')
        { // read one key which is a company name(including a blank).
            name[i] = line[k];
            k++;  i++;
        }
        name[i] = '\0';
